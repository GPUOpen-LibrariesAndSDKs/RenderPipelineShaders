...
#define RPS_AFX_REQUIRE_IMGUI  1

#include "test_visualizer_shared.hpp"
...
class TestVkRenderVisualizer : public RpsTestVulkanRenderer, public TestRpsRenderVisualizer
{
protected:
    virtual void OnInit(VkCommandBuffer initCmdBuf, InitTempResources& tempResources) override
    {
        ...
        initInfo.QueueFamily               = m_rpsQueueIndexToVkQueueFamilyMap[RPS_AFX_QUEUE_INDEX_GFX];
        initInfo.Queue                     = m_queues[RPS_AFX_QUEUE_INDEX_GFX];
        ...
        attachment.loadOp  = TestRpsRenderVisualizer::s_UseRenderToTextureImplementation ? VK_ATTACHMENT_LOAD_OP_LOAD
                                                                                         : VK_ATTACHMENT_LOAD_OP_CLEAR;
        attachment.storeOp = VK_ATTACHMENT_STORE_OP_STORE;
        attachment.stencilLoadOp  = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
        attachment.stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
        attachment.initialLayout  = TestRpsRenderVisualizer::s_UseRenderToTextureImplementation
        ...
        VkResult result             = vkCreateRenderPass(m_device, &info, nullptr, &m_imguiRenderPass);
        ...
        TestRpsRenderVisualizer::OnInit();
    ...
    {
        TestRpsRenderVisualizer::OnCleanUp();
    ...
    {
        TestRpsRenderVisualizer::OnUpdate(frameIndex, m_width, m_height);
        UpdatePipeline(frameIndex, CalcGuaranteedCompletedFrameIndexForRps());
    }

    virtual void OnRender(uint32_t frameIndex) override
    {
        REQUIRE(RPS_SUCCEEDED(ExecuteRenderGraph(frameIndex, GetRpsRenderGraph(), true, false)));

        StartImGuiDraw();

        REQUIRE(RPS_SUCCEEDED(rpsVisualizerDrawImGui(m_rpsVisualizer)));

        REQUIRE(RPS_SUCCEEDED(FinishImGuiDraw(GetRpsRenderGraph())));
...
protected:
    ...
    virtual void CreateRpsNullRuntimeDevice(RpsDevice& rpsDeviceOut) override final
    {
        rpsDeviceOut = rpsTestUtilCreateNullRuntimeDevice();
    }
    ...
    {
        auto pThis = static_cast<TestVkRenderVisualizer*>(pContext->pCmdCallbackContext);
    ...
    virtual LRESULT WindowProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam, bool& bHandled) override
    {
...
private:
    ...
    void UpdatePipeline(uint64_t frameIndex, uint64_t completedFrameIndex)
    {
        ...
        if (m_swapChainImages.size() > RPS_COUNTOF(backBuffers))
            throw;
        ...
        TestRpsRenderVisualizer::UpdateRpsPipeline(frameIndex, completedFrameIndex, backBufferDesc, backBuffers);
    }
...
TEST_CASE(TEST_APP_NAME)
{
    ...
    TestVkRenderVisualizer renderer;
